...
class WeightedSoftTripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(n):
            if i < n/2:
                dist_ap = dist[i][int(n/2):n][mask[i][int(n/2):n]].unsqueeze(0)  # dist[i][mask[i]].max()得到的tensor没有维度，需要加unsqueeze(0)得到一维向量
                dist_an = dist[i][int(n/2):n][(mask[i] == 0)[int(n/2):n]].unsqueeze(0)
            else:
                dist_ap = dist[i][0:int(n/2)][mask[i][0:int(n/2)]].max().unsqueeze(0)  # dist[i][mask[i]].max()得到的tensor没有维度，需要加unsqueeze(0)得到一维向量
                dist_an = dist[i][0:int(n/2)][(mask[i] == 0)[0:int(n/2)]].min().unsqueeze(0)
            ...
            singleloss = torch.log(1+torch.exp(ap_-an_))
            loss += singleloss
        loss = loss/n*2
        return loss
